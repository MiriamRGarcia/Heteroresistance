...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [xT, sens_xT] = Sens_MultiExp(tmod, rr, Cexp, par, x0, ODEoptions)
...
% Tranformate parameters of the initial condition:
%X0      = 10^Y0;
%lambIC  = - log(lambyIC);
...
% f0_aux     = exp(-lambIC*rr);
% s0_Y0      = log(10)*X0*f0_aux/sum(f0_aux);
...
%RR_aux     = repmat(rr.', nr, 1) - repmat(rr, 1, nr);    
%s0_lambIC  = X0/(sum(f0_aux)^2)*f0_aux.*(RR_aux*f0_aux);
s0_lambIC = X0*f0_aux.*(f0_aux.'*rr/sum(f0_aux) - rr)/sum(f0_aux);
