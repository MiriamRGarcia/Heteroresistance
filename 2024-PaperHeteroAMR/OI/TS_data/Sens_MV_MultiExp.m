...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [xT, varxT, sens_xT, sens_varxT, munT, sens_munT] = Sens_MV_MultiExp(tmod, rr, Cexp, par, x0, ODEoptions)
...
for iexp = 1:Nexp
    ...
    [~, St_sens] = ode15s(@(t,s) Odes_SensMV(t, s, AA, AA_mugS, AA_mugR, AA_alphg,...
    ...
    mun              = mug - muk;
    munT(1:nt, iexp) = xx*mun./xT(1:nt, iexp);
    ...
    % Sensitivities of the total net-growth rate:
    sens_mun                 = der_mug - der_muk;
    
    for ip = 1:np
        sens_munT(1:nt, ip, iexp) = ...
            (xx*sens_mun(1:nr, ip) + xx*mun - sens_xT(1:nt, ip, iexp)./xT(1:nt, iexp).*(xx*mun))./xT(1:nt, iexp);
